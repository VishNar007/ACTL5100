...
```{r read}
benefit_df <- read_csv("../Data/Processed Data/Benefit_Modelling.csv")
rate_df <- read_excel("../Data/Case Study Data/srcsc-2024-lumaria-economic-data.xlsx", skip=10)
```

```{r}
# Original parameter values
orig_inflation_rate <- mean(rate_df$Inflation)
orig_interest_rate <- mean(rate_df$`1-yr Risk Free Annual Spot Rate`)
orig_engagement_rate <- 0.25
```
...
sens_results <- tibble(Inflation_Rate_Change = factor(),
                       Interest_Rate_Change = factor(),
                       Engagement_Rate_Change = factor(),
                       T20_Profit_Count = numeric(),
                       SWPL_Profit_Count = numeric())
```
...
for (inflation_change in sens_levels) {
  for (interest_change in sens_levels) {
    for (engagement_change in sens_levels) {
      ...
      new_inflation_rate  <- orig_inflation_rate * (1 + inflation_change)
      new_interest_rate   <- orig_interest_rate * (1 + interest_change)
      new_engagement_rate <- orig_engagement_rate * (1 + engagement_change)
      ...
      SWPL_EPV_df_sens <- benefit_model(120, mortality_df, new_interest_rate, new_inflation_rate) %>%
        mutate(EPV_single_adj = benefit_model(120, adj_mortality_df, new_interest_rate, new_inflation_rate)$EPV_single)
      
      # New intervention benefits
      T20_intervention <- ((1 - new_engagement_rate) * T20_EPV_df_sens$EPV_single 
                           + new_engagement_rate * T20_EPV_df_sens$EPV_single_adj) * T20_FV 
                           + new_engagement_rate * (T20_expense_df$smoking + T20_expense_df$screening + T20_expense_df$fitness + T20_expense_df$safety)
      
      SWPL_intervention <- ((1 - new_engagement_rate) * SWPL_EPV_df_sens$EPV_single
                            + new_engagement_rate * SWPL_EPV_df_sens$EPV_single_adj) * SPWL_FV 
                            + new_engagement_rate * (SPWL_expense_df$smoking + SPWL_expense_df$screening + SPWL_expense_df$fitness + SPWL_expense_df$safety)
      
      # Comparing benefits
      T20_profit_flag <- factor(ifelse(T20_intervention < benefit_df$T20_baseline, "Profit", "Loss"))
      SWPL_profit_flag <- factor(ifelse(SWPL_intervention < benefit_df$SPWL_baseline, "Profit", "Loss"))
      
      # Count profit occurrences
      T20_profit_count <- sum(T20_profit_flag == "Profit")
      SWPL_profit_count <- sum(SWPL_profit_flag == "Profit")
      
      # Append results to sensitivity results df
      sens_results <- sens_results %>%
        add_row(Inflation_Rate_Change = inflation_change*100,
          Interest_Rate_Change = interest_change*100,
          Engagement_Rate_Change = engagement_change*100,
          T20_Profit_Count = T20_profit_count,
          SWPL_Profit_Count = SWPL_profit_count)
    }
  }
}
```
